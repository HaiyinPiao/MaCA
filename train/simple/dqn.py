...
class RLFighter:
    ...
    def choose_action(self, info_obs):
        ...
        if np.random.uniform() < self.epsilon:
            actions_value = self.eval_net(info_obs)
    ...
    def learn(self):
        ...
        q_eval = self.eval_net(s_info_mem).gather(1, a_mem)  # shape (batch, 1)
        q_next = self.target_net(s__info_mem).detach()  # detach from graph, don't backpropagate
