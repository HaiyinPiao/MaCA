...
# print(6+INFO_LEN*N_R_VISIBLE+2+INFO_LEN*N_R_VISIBLE+2+INFO_LEN*N_STRK_D_L)
...
class ObsConstruct:
    ...
    def obs_construct(self, obs_raw_dict):
        ...
        for x in range(self.detector_num):
            ...
            detector_obs_list.append({'info': copy.deepcopy(data_context),
        ...
        for x in range(self.fighter_num):
            ...
            fighter_obs_list.append({'info': copy.deepcopy(data_context),
    ...
    def __get_data_obs(self, detector_data_obs_list, fighter_data_obs_list, joint_data_obs_dict):
        ...
        fighter_data = np.full((self.fighter_num, 6+INFO_LEN*N_R_VISIBLE+2+INFO_LEN*N_R_VISIBLE+2+INFO_LEN*N_STRK_D_L), -1, dtype=np.int32)
        ...
        # fighter_data_obs_list[1]['r_visible_list'].append({'id': 655544,'type': 11111,'pos_x': 2222,'pos_y': 3333})

        # Fighter info
        for x in range(self.fighter_num):
            if fighter_data_obs_list[x]['alive']:
                # print(fighter_data_obs_list[x])
                # exit()
                fighter_data[x, 0] = fighter_data_obs_list[x]['id']
                ...
                # fill r_visible_list.
                # first fill with 0.
                # then fill with available data.
                for i,j in zip(range(6,6+INFO_LEN*N_R_VISIBLE,N_R_VISIBLE),range(len(fighter_data_obs_list[x]['r_visible_list']))):
                    fighter_data[x, i] = fighter_data_obs_list[x]['r_visible_list'][j]['id']
                    fighter_data[x, i+1] = fighter_data_obs_list[x]['r_visible_list'][j]['type']
                    fighter_data[x, i+2] = fighter_data_obs_list[x]['r_visible_list'][j]['pos_x']
                    fighter_data[x, i+3] = fighter_data_obs_list[x]['r_visible_list'][j]['pos_y']

                idx = 6+INFO_LEN*N_R_VISIBLE
                fighter_data[x, idx] = fighter_data_obs_list[x]['j_iswork']
                fighter_data[x, idx+1] = fighter_data_obs_list[x]['j_fre_point']

                idx += 2
                for i,j in zip(range(idx,idx+INFO_LEN*N_R_VISIBLE,N_R_VISIBLE),range(len(fighter_data_obs_list[x]['j_recv_list']))):
                    fighter_data[x, i] = fighter_data_obs_list[x]['j_recv_list'][j]['id']
                    fighter_data[x, i+1] = fighter_data_obs_list[x]['j_recv_list'][j]['type']
                    fighter_data[x, i+2] = fighter_data_obs_list[x]['j_recv_list'][j]['direction']
                    fighter_data[x, i+3] = fighter_data_obs_list[x]['j_recv_list'][j]['r_fp']

                idx = 6+INFO_LEN*N_R_VISIBLE+2+INFO_LEN*N_R_VISIBLE
                fighter_data[x, idx] = fighter_data_obs_list[x]['l_missile_left']
                fighter_data[x, idx+1] = fighter_data_obs_list[x]['s_missile_left']

                idx += 2
                for i,j in zip(range(idx,idx+INFO_LEN*N_STRK_D_L,N_STRK_D_L),range(len(fighter_data_obs_list[x]['striking_dict_list']))):
                    fighter_data[x, i] = fighter_data_obs_list[x]['striking_dict_list'][j]['target_id']
                    fighter_data[x, i+1] = fighter_data_obs_list[x]['striking_dict_list'][j]['pos_x']
                    fighter_data[x, i+2] = fighter_data_obs_list[x]['striking_dict_list'][j]['pos_y']
                    fighter_data[x, i+3] = fighter_data_obs_list[x]['striking_dict_list'][j]['type']
